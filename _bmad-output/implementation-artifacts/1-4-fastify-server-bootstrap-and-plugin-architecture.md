...
- Internal packages: `import { ... } from '@mycscompanion/shared'` — no `.js` extension needed
...
- dotenv-cli installed in both `@mycscompanion/shared` and `@mycscompanion/backend` devDeps
...
- ESLint config extends from `@mycscompanion/config`
...
- `@mycscompanion/webapp:build` fails with `Can't resolve 'tailwindcss'` — pre-existing issue unrelated to this story. Backend build succeeds independently.
