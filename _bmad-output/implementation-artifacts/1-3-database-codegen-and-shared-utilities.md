...
5. Generated types are importable by all apps and packages via `import type { DB } from '@tycs/shared'`.
...
- [x] Task 1: Install kysely-codegen in `packages/shared` (AC: #1, #4)
  - [x] 1.1 Install dev deps: `pnpm --filter shared add -D kysely-codegen kysely pg`
    ...
    - These are dev-only — `@tycs/shared` has NO runtime database deps
...
- [x] Task 4: Wire generated types into shared package exports (AC: #5)
  ...
  - [x] 4.5 Verify: `import type { DB, Users, Tracks, Milestones } from '@tycs/shared'` works from any app
...
- [x] Task 5: Update backend `db.ts` to use generated types (AC: #1)
  - [x] 5.1 Modify `apps/backend/src/shared/db.ts`:
    ...
    - Import: `import type { DB } from '@tycs/shared'`
...
- `.env.example` — `DATABASE_URL=postgresql://tycs:tycs@localhost:5433/tycs`
...
- Installed kysely-codegen, kysely, pg, dotenv-cli as devDeps in `@tycs/shared`
...
1. **HIGH — Seed script hardcoded slugs instead of shared constants** (Fixed)
   - `seed.ts` now imports `TRACKS`/`MILESTONES` from `@tycs/shared` instead of duplicating string literals
